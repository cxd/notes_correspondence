---
title: "Notes on Correspondance Analysis"
author: "Chris Davey"
date: "24/12/2019"
output:
...
The method for obtaining the correspondance matrix and the associated chisq distances, as well as principle and standard coordinates is derived from chapter 17 of "Modern Multivariate Statistical Techniques" by Alan Izenman  @izenman.

The approach to performing the correspondance analysis converts each record into two matrices of one-hot encoded records for each factor $X$ for a factor representing rows, and $Y$ for a factor representing columns. For example a sample of the X matrix for Land Use.
...
N = X'Y
$$
The matrix $N$ contains the frequency where the corresponding occurance of an indicator in $X$ occurs with an indicator in $Y$. In this data set it is the count of the frequency where a Land Use category in $X$ occurs in correspondance with a mercury level band from $Y$.
...
In the example data set the set of matrices are calculated using the convenience method from the ca_tools.R script "compute_correspondance_tables". 
...
The correspondance analysis provides unbiased estimators for joint and conditional densities, and is able to test for independence of rows and columns. In addition the principle coordinates produced provide an ordination that can be useful in visualising correspondances between row and column categories and along with the measures of inertia can aid in the interpretation of the data.
